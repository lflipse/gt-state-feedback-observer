...
class DynamicsModel:
    ...
    def RK4(self, x, u, uh, h):
        k1x = h * self.xdot(x, u, uh)
        k2x = h * self.xdot(x + 0.5 * k1x, u, uh)
        k3x = h * self.xdot(x + 0.5 * k2x, u, uh)
        k4x = h * self.xdot(x + k3x, u, uh)

        # Update next value of y
        x_new = x + (1.0 / 6.0) * (k1x + 2 * k2x + 2 * k3x + k4x)
        return x_new
    ...
    def simulate(self, N, h, r, x0, u0, uh0, C, Qh0, R, GT):
        Q = C - Qh0
...
# Qh0 = np.array([[Qh_e, 0],[0, Qh_v]])
u_LQ0, uh_LQ0, x_LQ0, Lhe_LQ0, Lhv_LQ0 = dynamics_model.simulate(N, h, r, x0, u0, uh0, C, Qh0, R, GT=0)
u_GT0, uh_GT0, x_GT0, Lhe_GT0, Lhv_GT0 = dynamics_model.simulate(N, h, r, x0, u0, uh0, C, Qh0, R, GT=1)
...
#     Qh0 = np.array([[Qh_e, 0],[0, Qh_v]])
#     u_LQ[i, :], uh_LQ[i, :], x_LQ[i, : ,:], Lhe_LQ[i], Lhv_LQ[i] = dynamics_model.simulate(N, h, r, x0, u0, uh0, C, Qh0, R, GT=0)
#     u_GT[i, :], uh_GT[i, :], x_GT[i, :, :], Lhe_GT[i], Lhv_GT[i] = dynamics_model.simulate(N, h, r, x0, u0, uh0, C, Qh0, R, GT=1)
#
# plt.figure(3)
